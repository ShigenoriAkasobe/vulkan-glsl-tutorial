...
static Mat4 mat4RotationZ(float radians) {
    Mat4 r = mat4Identity();
...
private:
    ...
    void initWindow() {
        ...
        window = glfwCreateWindow(kWindowWidth, kWindowHeight, "Step05_LightingBasic", nullptr, nullptr);
        if (!window) {
    ...
    void createGraphicsPipeline() {
        auto vertShaderCode = readFile(shaderPath("lighting.vert.spv"));
        auto fragShaderCode = readFile(shaderPath("lighting.frag.spv"));
    ...
    void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer,
        ...
        vkCheck(vkAllocateMemory(device, &allocInfo, nullptr, &bufferMemory), "vkAllocateMemory");
        vkCheck(vkBindBufferMemory(device, buffer, bufferMemory, 0), "vkBindBufferMemory");
    }
    ...
    void createUniformBuffers() {
        ...
        for (size_t i = 0; i < swapchainImages.size(); ++i) {
            ...
            vkCheck(vkMapMemory(device, uniformBufferMemories[i], 0, bufferSize, 0, &uniformBufferMapped[i]),
                    "vkMapMemory(ubo)");
        }
    }
    ...
    void updateUniformBuffer(uint32_t imageIndex) {
        ...
        UBO ubo{};
        ubo.mvp = mat4RotationZ(t);
        ubo.lightDir[0] = -0.3f;
        ubo.lightDir[1] = -0.7f;
        ubo.lightDir[2] = -1.0f;
        ubo.lightDir[3] = 0.0f;

        ubo.baseColor[0] = 1.0f;
        ubo.baseColor[1] = 0.8f;
        ubo.baseColor[2] = 0.3f;
        ubo.baseColor[3] = 1.0f;

        std::memcpy(uniformBufferMapped[imageIndex], &ubo, sizeof(ubo));
    }
    ...
    void createDescriptorSets() {
        ...
        vkCheck(vkAllocateDescriptorSets(device, &allocInfo, descriptorSets.data()), "vkAllocateDescriptorSets");
