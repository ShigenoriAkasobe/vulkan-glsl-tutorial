...
private:
    ...
    void initWindow() {
        ...
        window = glfwCreateWindow(kWindowWidth, kWindowHeight, "Step03_Texture", nullptr, nullptr);
        if (!window) {
    ...
    void createGraphicsPipeline() {
        auto vertShaderCode = readFile(shaderPath("texture.vert.spv"));
        auto fragShaderCode = readFile(shaderPath("texture.frag.spv"));
    ...
    VkCommandBuffer beginSingleTimeCommands() {
        ...
        vkCheck(vkAllocateCommandBuffers(device, &allocInfo, &commandBuffer), "vkAllocateCommandBuffers(single)");

        VkCommandBufferBeginInfo beginInfo{};
        beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
        beginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;

        vkCheck(vkBeginCommandBuffer(commandBuffer, &beginInfo), "vkBeginCommandBuffer(single)");
        return commandBuffer;
    }

    void endSingleTimeCommands(VkCommandBuffer commandBuffer) {
        vkCheck(vkEndCommandBuffer(commandBuffer), "vkEndCommandBuffer(single)");
        ...
        vkCheck(vkQueueSubmit(graphicsQueue, 1, &submitInfo, VK_NULL_HANDLE), "vkQueueSubmit(single)");
        vkCheck(vkQueueWaitIdle(graphicsQueue), "vkQueueWaitIdle");
    ...
    void createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage,
        ...
        vkCheck(vkAllocateMemory(device, &allocInfo, nullptr, &imageMemory), "vkAllocateMemory(image)");
        vkCheck(vkBindImageMemory(device, image, imageMemory, 0), "vkBindImageMemory");
    }
    ...
    void createTextureImage() {
        ...
        for (uint32_t y = 0; y < texHeight; ++y) {
            for (uint32_t x = 0; x < texWidth; ++x) {
                const bool checker = ((x / 32) % 2) ^ ((y / 32) % 2);
                const uint8_t c = checker ? 220 : 40;
                const size_t idx = static_cast<size_t>((y * texWidth + x) * 4);
                pixels[idx + 0] = c;
                pixels[idx + 1] = c;
                pixels[idx + 2] = c;
                pixels[idx + 3] = 255;
        ...
        void* data = nullptr;
        vkCheck(vkMapMemory(device, stagingBufferMemory, 0, imageSize, 0, &data), "vkMapMemory(staging)");
    ...
    VkImageView createImageView(VkImage image, VkFormat format) {
        ...
        vkCheck(vkCreateImageView(device, &viewInfo, nullptr, &imageView), "vkCreateImageView(texture)");
        return imageView;
    }
    ...
    void createTextureSampler() {
        ...
        vkCheck(vkCreateSampler(device, &samplerInfo, nullptr, &textureSampler), "vkCreateSampler");
    }
    ...
    void createDescriptorSet() {
        ...
        vkCheck(vkAllocateDescriptorSets(device, &allocInfo, &descriptorSet), "vkAllocateDescriptorSets");
